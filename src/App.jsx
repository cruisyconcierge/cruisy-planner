...
const fetchRealActivities = async (destination) => {
  try {
    // 1. Fetch the Location Hub (CPT: 'location' -> Endpoint: 'locations')
    const destRes = await fetch(`https://cruisytravel.com/wp-json/wp/v2/locations?search=${destination}&acf_format=standard`);
    const destData = await destRes.json();
    
    // Default fallback if destination not found in WP
    const hub = destData.length > 0 ? destData[0] : {}; 
    const acf = hub.acf || {};

    // 2. Fetch the Activities (CPT: 'itinerary' -> Endpoint: 'itineraries')
    ...
    // 3. Map WordPress Data to App Structure
    const mappedActivities = actData.map(post => {
      ...
      return {
        ...
        description: post.content.rendered, // Keep HTML for detail view
        bookingUrl: post.acf.booking_url || "#",
        tags: [] 
      };
    });

    // 4. Construct Dynamic Links (Using ACFs from 'Location' CPT)
    const stayPartners = [
      { 
        name: "Vrbo", 
        icon: Home, 
    ...
    // HARDCODED GEAR (Global Defaults)
    const products = [
    ...
    return {
      ...
      products: products, 
      weather: { temp: 82, condition: 'Sunny', icon: Sun }, // Weather is mocked
    };
...
const Button = ({ children, onClick, variant = 'primary', className = '', type='button', disabled = false, fullWidth = false, style={} }) => {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled} className={`${baseStyle} ${variants[variant]} ${className}`} style={mergedStyle}>{children}</button>
  );
};
...
export default function App() {
  ...
  const handleSearch = async (e) => {
    ...
    // Initialize Essentials Checklist
    setEssentials([
  ...
  const toggleBooked = (id, type = 'activity') => {
    if (type === 'activity') {
        ...
        <form onSubmit={handleSearch} className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="relative">
              <input required type="text" placeholder="e.g. Key West, Miami, Cozumel..." className="w-full pl-4 pr-12 text-xl border-2 border-gray-100 rounded-lg py-4 focus:border-[#34a4b8] outline-none font-medium transition-colors" value={destinationSearch} onChange={(e) => setDestinationSearch(e.target.value)} />
